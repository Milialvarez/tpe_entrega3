...
class ControllerCategorias extends apiController{
    ...
    public function listarCategoria($params =[]){
        if(empty($params)){
            ...
            $id = $params[':ID'];
            $categoriaId = $this->model->getCategoriaPorId($id); //categria por id
            if($categoriaId){
                $this->view->response($categoriaId, 200);  
            } else {
    ...
    public function modificarCategoria($params = []){
        ...
        if($categoriaID){
            ...
            if (!empty($nombre) && !empty($descripcion)){
                ...
                $this->view->response('Complete los campos', 401);
            }
        } else {
    ...
    public function eliminarCategoria($params = []){
        ...
        if($productoId){
            ...
            $this->view->response('No se pudo eliminar la categoria con el id' . $id, 404);
        }
    }
}
?>