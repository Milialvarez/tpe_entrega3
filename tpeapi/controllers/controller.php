...
class Controller extends apiController{
    ...
    public function agregarProducto($params = []){
        ...
        if (!empty($producto) && !empty($id_categoria) && !empty($stock) && !empty($precio) && !empty($proveedor) && !empty($descripcion) && !empty($imagen) && !empty($oferta) && !empty($tendencia) && isset($oferta) && isset($tendencia)){
            ...
            $this->view->response('Se ha agregado con éxito', 201);
        } else {
    ...
    public function eliminarProducto($params = []){
        ...
        if($productoId){
            ...
            $this->view->response('No se pudo eliminar ya que no existe un producto con el id ' . $id, 404);
        }
    }
    ...
    public function listarProductosPrecio($params = []){
        ...
        if($precio1<$precio2){
            ...
            $productos = $this->model->getProductosPorPrecio($precio2, $precio1);
        } else {
            $this->view->response("ingrese dos precios diferentes", 400);
            return;
        }
        if ($productos) {
            $this->view->response($productos, 200);
    ...
    public function getProductosCategoria($params=[]){
        if(!empty($params)){
            ...
            if($existe){
                ...
                $this->view->response("no existe la categoría enviada", 404);
            }
        } else {
